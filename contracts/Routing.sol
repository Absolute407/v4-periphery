...
abstract contract UniswapV4Routing {
    ...
    function v4Swap(SwapInfo memory swapInfo) internal {
    ...
    function _swapExactInput(ExactInputParams memory params) private {
        for (uint256 i = 0; i < params.path.length; i++) {
            ...
            if (i == 0) {
                if (zeroForOne) {
                    _pay(Currency.unwrap(poolKey.currency0), msg.sender, address(poolManager), uint256(uint128(delta.amount0())));
                    poolManager.settle(poolKey.currency0);
                } else {
                    _pay(Currency.unwrap(poolKey.currency1), msg.sender, address(poolManager), uint256(uint128(delta.amount1())));
            ...
            if (i == params.path.length - 1) {
                if (zeroForOne) {
                    poolManager.take(poolKey.currency1, msg.sender, uint256(uint128(delta.amount1())));
                } else {
                    poolManager.take(poolKey.currency0, msg.sender, uint256(uint128(delta.amount0())));
                }
            }
